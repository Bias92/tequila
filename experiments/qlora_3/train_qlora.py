...
def build_chat(example, tokenizer):
    ...
    Apply completion-only loss mask: labels = -100 for the user/system portion."""
    user_msgs = [
    ...
    user_enc = tokenizer(user_text, add_special_tokens=False)
...
def main():
    ...
    # Completion-only loss: labels are pre-masked in build_chat (-100 for user portion).
    # Custom collator pads input_ids/attention_mask/labels consistently.
